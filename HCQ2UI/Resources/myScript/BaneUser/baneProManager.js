...
var $userName = "hiden_name", $userIdentCode = "hiden_identify";
...
var baneProManager = {
	        ...
	        },
            {
                ...
                formatter: function (value, row, index) {
                    if (row.approve_status===0)
                        return "<button onclick=baneProManager.editBaneProDataById('" + value + "','"+row.user_name+"') class='btn btn-info btn-xs' type='button'><i class='fa fa-paste'></i>办 理</button>";
                    return "";
                }
            },
	        {
    ...
    bindEvent: function () {
        ...
        $('#baneConduct').click(function () {
            ...
            baneProManager.recordForm(row[0].user_identify);
        });
    },
    //办理尿检登记
    editBaneProDataById:function(ur_id,user_name){
        //先获取待编辑数据
        $.ajax({
            type: 'post',
    ...
    //查看历史记录
    recordForm: function (user_identify) {
        layer.open({
            title: ['定期检测记录一栏', 'font-size:18px;'],
            type: 2,
            content: ctx + '/BaneUser/UrinalysisList?user_identify=' + user_identify+'&type=record',
            scroll: true,//是否显示滚动条、默认不显示
            btn: '',
            area: ['1000px', '95%'],
            cancel: function (li, o) {
